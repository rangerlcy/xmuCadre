 ...
 <!-- ------------------------------------------------------------------------------------------------------------------------------- -->	
 	...
 	<script>
	 						...
	 						$.post("${ctx}/userTrain/UserTrainInfo/showTrainUserSimple.do",{userId:userId,trainId: trainId},function(resultOUI){
	 							...
	 							if (resultOUI.positionSimple.id < 0){
		 							...
		 							userInfo+="<tr><td  class=\"haveChoose\" value='" +  resultOUI.userId + "'>"
		 								...
		 								+ "<a class=\"userDetail\" onclick=\" seeDetail(this)\">详情</a></td>"
		 								+"</tr>";
 		...
 		function seeDetail(obj){
 			var id = $(obj).parent().attr("tid");
		 	art.dialog.open("${ctx}/infoManager/userInfo/openUserDetail.do?id="+id,{ title: "用户详情", lock: true, width: 996, height: 600});
 		};
	</script>
