...
<html>
  ...
  <body>
  <div class="form_head"><span class="moreDel">批量删除教育经历</span></div>
    	...
    	<table class="tblist" id="tab">
				...
				<c:forEach items="${queryList.result }" var="uv">
					<tr>
						...
						<td>${uv.school }</td>
						<td>${ide:getItem('learningForm',uv.learningForm) }</td>
						<td>${ide:getItem('learningPhase',uv.learningPhase) }</td>
						<td>${ide:getItem('learningState',uv.learningState) }</td>
						<td>${ide:getItem('education',uv.degree) }</td>
						<td>${ide:getItem('degreeType',uv.degreeType) }</td>
