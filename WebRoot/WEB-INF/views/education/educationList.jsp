...
<html>
  ...
  <body>
  <div class="form_head"><span class="showList">教育经历信息列表</span> <span class="moreDel" style="display:none">批量删除教育经历</span></div>
  	<c:if test="${roleType=='admin' }">
	<input type="button" value="添加教育经历" class="btn addBtn showList"/>
	</c:if>

  <div class="educationShow">
  		<form id="searchUserForm" action="${ctx}/education/educationList.do" method="post">
    	...
    	<table class="tblist" id="tab">
				...
				<c:forEach items="${queryList.result }" var="uv">
					<tr>
						...
						<td>${uv.school }</td>
						<td>${uv.learningForm }</td>
						<td>${uv.learningPhase }</td>
						<td>${uv.learningState }</td>
						<td>${uv.degree }</td>
						<td>${uv.degreeType }</td>
						<td>${uv.checkCase }</td>
						<td>${uv.remark }</td>
	...
	//添加
		function bindAddEvent() {
			$(".addBtn").click(function(){
				art.dialog.open(
					"${ctx}/education/openEducationAdd.do",
					{ title: "添加教育经历", lock: true, width: 1000, height: 650 }
				);
			});		
		}
	//修改
	function bindEditEvent() {
			...
			var id = thiz.parent().attr("TID");
				art.dialog.open(
					"${ctx}/education/openEducationEdit.do?id="+id,
					{ title: "修改教育经历", lock: true, width: 1000, height: 650 }
				);
			});		
		}
	...
	function bindOpenManyDelEvent(){
		$(".manyDelBtn").click(function(){
			window.location.href="/xmuCadre/education/openEducationMoreDel.do";
		});
	}
	...
	function bindCloseManyDelEvent(){
		$(".backBtn").click(function(){
			window.location.href="/xmuCadre/education/educationList.do";
		});
	}
	...
	function bindManyDelEvent(){
		$(".manyDel").click(function(){
			...
			art.dialog.confirm("确定删除本页面被选中的教育经历？",function(){
					$.post("${ctx}/education/educationManyDel.do?str="+str,
						function(result){
			                if("success" == result) {
		...
		function bindDelEvent() {
			$(".delBtn").click(function(){
				...
				art.dialog.confirm("确定删除？",function(){
					$.post("${ctx}/education/educationDel.do",{id:id},
						function(result){
			                if("success" == result) {
			                	art.dialog.tips("删除成功");
			                	art.dialog.opener.location.reload();             
