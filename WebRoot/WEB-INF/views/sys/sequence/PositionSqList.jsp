...
<span style="color:red; text-align:center">父级单位：${queryList.result[0].department }</span>
<div class="departmentSequece">
 		<form id="searchDepartmentForm" action="${ctx }/sys/sequence/openPositionNameSequence.do" method="post">
			<input type="text" id="queryStr" class="txt txtkey" name="queryStr" value="" style="width:220px;"   maxlength=40/>
			<input type="text" id="queryStrTemp" class="txt txtkey" value="岗位名称" style="width:220px;"/>
			<input value="${queryList.result[0].department }" name="departmentName" style="display:none">
    	...
    	<table class="tblist" >
				...
				<c:forEach items="${queryList.result }" var="uv">
					<tr>
						<c:if test="${uv.positionNameSeq == null}">
							<td>未设置顺序</td>
						</c:if>
						<c:if test="${uv.positionNameSeq != null}">
							<td>${uv.positionNameSeq }</td>
						</c:if>	
						<td>${uv.positionName }</td>
						<td class="c" TID="${uv.id }" NN="${uv.positionName }" NUM="${uv.positionNameSeq }">
