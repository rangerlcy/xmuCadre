...
public class TableMappingService {
	...
	public String getDbName(String str,int i) {
		...
		if(str==null){
			...
			if(i==1){		//返回字典表对应的民族
				value = ViewFunction.getEthnicityName(str);
			}
			if(i==2){		//返回字典表对应的党派
				value = ViewFunction.getPolicitalStatusName(str);
			}
			if(i==3){		//返回字典表对应的学位
				value = ViewFunction.getDegreeName(str);
			}
			if(i==4){		//返回人员级别
				value = ViewFunction.getLevel(str);
			}
			if(i==5){		//返回岗位名称
				value = ViewFunction.getPositionName(str);
			}
			return value;
		}
	}
	...
	//时间转字符串
	public String Date2Str(Date dd){
		String day="";
		if(dd==null){
			return day;
		}else{
