...
public class AppointAndDismissService {
	...
	public Page<WorkingPaper> findAADByPage(String queryStr,int currPage,int pageSize) throws Exception{
		...
		Object[] args = null;
		/*if(flag==true){
			//sql="from WorkingPaper as wp where wp.id in(select workingPaperId from PostUser,User where PostUser.id = User.id and User.Name = queryStr)";
			where+= "and (wp.id in (select workingPaperId from PostUser where PostUser.user.name like)";
		}
		else{*/
	...
	public List<PostUser> findByWorkingPaperId(Integer id) {
		// TODO Auto-generated method stub
		String hql ="from PostUser where workingPaper.id = ?";
