...
public class UserService {
	 ...
	 * User List
	 */
	public List<User> findUser(String queryStr) throws Exception{
		if (StringUtil.isNotBlank(queryStr))
			queryStr = queryStr.replace(" ", "");
		String sql = "from User as ur   ";//查询对象
		String where = " (ur.delFlag is null or ur.delFlag=1) and ur.name<>'admin' ";//查询条件
		...
		sql = sql + " where "+where;
		sql =sql + " order by ur.id desc";
		
		List<User> users = UserDao.queryList(sql, args);
		return users;
	}
	/*
	 * 高级查询，不是分页查询
	 * 返回User List
	 */
	public List<User> advanceSearchUser(String name,String sex,String idNumber,Date birthday,String birthdayOperator,
			String age,String ageOperator,String positionName,String positionNameOperator)throws Exception{
	...
	//*********************************************更新教育经历
	public boolean updateEducationHistory(List<EducationHistory> educationHistoryEdit){
		for(int i=0;i<educationHistoryEdit.size();i++)
		{
			if(educationHistoryEdit.get(i).getId() == null){
				return false;
			}
			educationHistoryDao.update(educationHistoryEdit.get(i));
		}
		return true;
	}
	//更新家庭人员
	public boolean updateRelation(List<Relation> relationEdit){
		for(int i=0;i<relationEdit.size();i++){
			
			if(relationEdit.get(i).getId()==null){
				return false;
			}
			relationDao.update(relationEdit.get(i));
		}
		return true;
	}
	/**
	...
	public List<User> queryUserByageAndLevel(String a, String b, String level) {
		// TODO Auto-generated method stub
		int age1=0,age2=0;
		if(StringUtil.isNotBlank(a)){
			age1= Integer.parseInt(a);
			age2= Integer.parseInt(b);
		}
		...
		bir1 = calendar.getTime();				//较大的出生日期，比如1985
		calendar.setTime(today);
		calendar.add(calendar.YEAR, -age2);
		bir2 = calendar.getTime();				//较小的出生日期, 比如1980
		System.out.println(bir1);
		System.out.println(bir2);
		List<User> result = new ArrayList<User>();
		if(StringUtil.isBlank(a) && StringUtil.isBlank(b) && StringUtil.isBlank(level)){		//所有的查询关键词为空
			String hql = "from User u ";
			hql=hql+" where (u.level is not null) and (u.retireDay is null or u.retireDay >?)";
			result = UserDao.queryList(hql,new Object[]{today});
		}
		else if( StringUtil.isNotBlank(a) && StringUtil.isBlank(level)){		//年龄不空，级别为空
			String hql = "from User u where u.birthDay>=? and u.birthDay<=? ";
			hql=hql+" and (u.level is not null) and (u.retireDay is null or u.retireDay >?)";
			result = UserDao.queryList(hql,new Object[]{ bir2, bir1, today});
		}
		else if(StringUtil.isBlank(a) && StringUtil.isBlank(b) && StringUtil.isNotBlank(level)){		//年龄空，级别不空
			String hql = "from User u where u.level=? ";
			hql=hql+" and (u.level is not null) and (u.retireDay is null or u.retireDay >?)";
			result = UserDao.queryList(hql,new Object[]{level, today});
		}
		else{
			String hql = "from User u where u.birthDay>=? and u.birthDay<=? and u.level=? ";						//都不空
			hql=hql+" and (u.level is not null) and (u.retireDay is null or u.retireDay >?)";
			result = UserDao.queryList(hql,new Object[]{bir2, bir1, level, today});
		}
		return result;
	}
