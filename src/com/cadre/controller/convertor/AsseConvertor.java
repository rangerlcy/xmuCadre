...
public class AsseConvertor extends AbstractCheckRepeatExcelDataConvertor<Assess>{
	private Logger logger = LogManager.getLogger(AsseConvertor.class);
	...
	private static final String assessmentResult = "考核结果";
	/**
	 * 在数据库中已经存在的用户信息
	 */
	...
	public AsseConvertor(Map<String,Assess> asseMap,Map<String, User> userMap){ 
		this.asseMap = asseMap;
		this.userMap = userMap;
	}
	
	public AsseConvertor(List<Assess> assList,List<User> userList) {
	...
	public Assess convert(Row row) {
		...
		assess.setAssessmentResult(resultString);
		assess.setDelFlag(1);
		return assess;
	}
