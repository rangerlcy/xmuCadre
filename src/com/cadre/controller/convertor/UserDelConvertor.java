...
public class UserDelConvertor extends AbstractCheckRepeatExcelDataConvertor<User>{
	private Logger logger = LogManager.getLogger(UserDelConvertor.class);
	...
	public UserDelConvertor(Map<String,User> map){ 
		this.userMap = map;
	}
	
	public UserDelConvertor(List<User> list) {
		this(toMap(list));
	}
	...
	protected DataErrorMsg checkDataRow(Row row) {
		...
		/*if(StringUtils.isBlank(cardType)){
			...
			if (null == userMap.get(cardNum))
				sb.append("第"+(numIndex + 1)+"列用户不存在");
		//}
		...
		/*if( StringUtils.isNotBlank(genderStr)
				&& ViewFunction.getGenderCode(genderStr) == null) {
